...
spark = SparkSession.builder.appName("stedi-redis-cust-record").getOrCreate()
...
custjoinDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF,expr("""
    customer = email
...
#custjoinDF.writeStream.outputMode("append").format("console").start().awaitTermination()

custjoinDF.selectExpr("cast(customer as string) key","to_json(struct(*)) as value") \
            .writeStream \
            .format("kafka") \
            .option("kafka.bootstrap.servers","localhost:9092") \
